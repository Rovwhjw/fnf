...
class Replay
{
	...
	public function SaveReplay(notearray:Array<Dynamic>, judge:Array<String>, ana:Analysis)
	{
		...
		#if sys
		File.saveContent(Main.getDataPath() + "assets/replays/replay-" + PlayState.SONG.song + "-time" + time + ".kadeReplay", data);
	...
	{
		#if sys
		trace('loading ' + Main.getDataPath() + 'assets/replays/' + path + ' replay...');
		try
		{
			var repl:ReplayJSON = cast Json.parse(File.getContent(Main.getDataPath() + "assets/replays/" + path));
			replay = repl;
		}
		catch(e)
		{
