...
class PlayState extends MusicBeatState
{
	...
	public static var isEvacuate:Bool = false;
	...
	public var swingBeats:Array<Int> = [358, 378, 396, 424, 485, 508, 522, 534, 611, 615, 621, 628, 632, 638, 648, 652, 658, 672, 719, 736, 748, 756, 764, 768, 776, 782, 784, 788, 792, 796, 934, 956, 983, 1004, 1016, 1028, 1034, 1038, 1184, 1196, 1202, 1212, 1228, 1240, 1260, 1276, 1284, 1292, 1300, 1304, 1308, 1388, 1398, 1404, 1416, 1424, 1436, 1504, 1525, 1542, 1564, 1568, 1584, 1600, 1616, 1632, 1648, 1664, 1680, 1702, 1716, 1724, 1740, 1760, 1770, 1775, 1777, 1784, 1788, 1797, 1804, 1812, 1820, 1824, 1832, 1848, 1864, 1871, 1875, 1880, 1894, 1900, 1906, 1910, 1916, 1924, 1938, 1948, 1952, 1958, 1964, 1974, 1986, 1990, 1994, 1998, 2020, 2026, 2044, 2048, 2052, 2056, 2064, 2068, 2074, 2078, 2080, 2086, 2092, 2098, 2104, 2108, 2120, 2124, 2128, 2144, 2150, 2156, 2168, 2184, 2198, 2214, 2232, 2248, 2256, 2269, 2278, 2284, 2296, 2300, 2304, 2312, 2316, 2322, 2326, 2336, 2342, 2348, 2354, 2360, 2364, 2376, 2380, 2384, 2398, 2400, 2415, 2416, 2431, 2432, 2440, 2447, 2448, 2452, 2456, 2460, 2470, 2476, 2482, 2488, 2504, 2520, 2524, 2528, 2542, 2552, 2560, 2568, 2574, 2576, 2580, 2582, 2588];
	...
	override public function create()
	{
		...
		isEvacuate = (SONG.song.toLowerCase() == 'evacuate');
		...
		if (!PlayStateChangeables.Optimize)
		{
			switch (stageCheck)
			{
					...
					case 'evilstage':
				{
						defaultCamZoom = 0.6;
						curStage = 'evilstage';
						var bg:FlxSprite = new FlxSprite(-520, -535).loadGraphic(Paths.image('bg/trollgestage','shared'));
				...
				case 'stage':
				{
						defaultCamZoom = 0.6;
						curStage = 'stage';
						var bg:FlxSprite = new FlxSprite(-520, -535).loadGraphic(Paths.image('bg/stageback','shared'));
						bg.setGraphicSize(Std.int(bg.width * 1.01));
						bg.updateHitbox();
						if(FlxG.save.data.antialiasing)
							{
						...
						var stageFront:FlxSprite = new FlxSprite(-508, 800).loadGraphic(Paths.image('bg/stagefront','shared'));
		...
		if (isEvacuate)
			healthBar.createFilledBar(0xFF000000, 0xFF66FF33);
        else
	...
	public function generateSong(dataPath:String):Void
	{
		...
		var songPath = Main.getDataPath() + 'assets/data/' + songLowercase + '/';
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			{
				case 'troll':
					babyArrow.frames = Paths.getSparrowAtlas('troll_NOTE_assets');
	...
	override public function update(elapsed:Float)
	{
		...
		if (isEvacuate && storyDifficulty > 1)
		{
			if (health > 0.01)
				health -= 0.00015;
		}
		...
		if (curStep == 670 && curSong == 'Prepare')
		{
				iconP2.animation.play('coughtroll');
		}
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				{
					case 'sirtroll' | 'coughtroll':
						camFollow.y = dad.getMidpoint().y - 650;
						camFollow.x = dad.getMidpoint().x + 100;
					case 'trollge':
						camFollow.y = dad.getMidpoint().y - 315;
		...
		//FlxG.watch.addQuick("evacStep", (curStep - 110));
					...
					if (daNote.isSustainNote && daNote.wasGoodHit)
						...
						else
						{
							if (loadRep && daNote.isSustainNote)
							{
								...
								{
									if (!daNote.isSustainNote && daNote.noteStyle != 'warning')
									{
										health -= 0.10;
										vocals.volume = 0;
									}
									...
									else
									{
											...
											&& daNote.spotInLine != daNote.parent.children.length)
										{
											...
											if (daNote.parent.wasGoodHit && daNote.noteStyle != 'warning')
											{
												misses++;
												updateAccuracy();
											}
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					else
					{
						...
						{
                                                        var video = new WebViewPlayer('post-evacuate');                                                     
						...
						{
                                                        var video = new WebViewPlayer('troll');                                                     
                                                        video.finishCallback = function() {                                                               
                                                               FlxG.switchState(new Killer());             
                                                        }
						} 
					}
				...
				else
				{
					...
					switch (SONG.song.toLowerCase())
					{
						...
						{	
							inCutscene = true;
							paused = true;                                                       
                                                        var video = new WebViewPlayer('prepare');                                                     
                                                        video.finishCallback = function() {                                                               
                                                               FlxG.switchState(new PlayState());             
                                                        }
						}
						...
						case 'prepare':
						{
							inCutscene = true;
							paused = true;
                                                        var video = new WebViewPlayer('evacuate');                                                     
                                                        video.finishCallback = function() {                                                               
                                                               FlxG.switchState(new PlayState());             
                                                        }
						}
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		switch (daRating)
		{
			case 'shit':
				if (daNote.noteStyle == 'warning')
					health -= 69;  //dm the word fart cake and I will send you a dollar
				else
				{
					score = -300;
					combo = 0;
...
var upHold:Bool = false;
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void
	{
		if (!boyfriend.stunned)
		{
			...
			if (FlxG.save.data.accuracyMod == 1 && daNote.noteStyle != 'warning')
				totalNotesHit -= 1;
			...
			else if (daNote.noteStyle != 'warning')
				songScore -= 10;
			
			if (FlxG.save.data.missSounds && daNote.noteStyle != 'warning')
				{
	...
	override function stepHit()
	{
		...
		{
			remove(dad);
			dad = new Character(dad.x, dad.y, 'coughtroll');
			add(dad);
		}
	...
	override function beatHit()
	{
		...
		if (executeModchart && luaModchart != null)
		{
			...
			luaModchart.setVar('curStep', curStep);
			luaModchart.executeState('beatHit', [curStep]);
		}
		#end
		...
		/*if (SONG.song == 'Evacuate' && dad.curCharacter == "trollge")
			{
				if (swingBeats.contains((curStep - 110)))
				{
					dad.playAnim("singLEFT", true);
					FlxG.camera.shake(0.05, 0.15);
					boyfriend.playAnim("dodge", true);
